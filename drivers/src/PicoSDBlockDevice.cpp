...
#endif
    {
        uint8_t *dummy_tx = (uint8_t *)malloc(length);
        memset(dummy_tx, SPI_FILL_CHAR, length);
        _spi.write(dummy_tx, length, buffer, length);
        free(dummy_tx);
    }
...
#endif
    {
        uint8_t *dummy_rx = (uint8_t *)malloc(length);
        _spi.write(buffer, length, dummy_rx, length);
        free(dummy_rx);
    }
