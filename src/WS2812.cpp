...
#define WS2812_USE_PICO_SEMAPHORE  (1) /* if using pico SDK semaphore, otherwise use FreeRTOS semaphore */
#define WS2812_USE_PICO_ALARM      (1) /* if using pico SDK alarm (issue in SDK 2.0.0!), otherwise use FreeRTOS timeout. Note that RESET_TIME_US could not be handled by FreeRTOS, will be 1ms!  */
#define NEOC_USE_DMA      (1) /* if using pico SDK DMA  */
...
/* semaphore used to make a delay at the end of the transfer. Posted when it is safe to output a new set of values: */
  ...
  static alarm_pool_t *alarm_pool_ws2812 = alarm_pool_create(3, 1); /* create an alarm pool with 3 hardware alarms and 16 timers */
...
void dma_complete_handler(void) {
  if (dma_hw->ints0 & DMA_CHANNEL_MASK) { /* are we called for our DMA channel? */
    ...
    if (reset_delay_alarm_id!=0) { /* safety check: is there somehow an alarm already running? */
      alarm_pool_cancel_alarm(alarm_pool_ws2812, reset_delay_alarm_id); /* cancel it */
    }
    /* setup alarm to wait for the required latch-in time for the RES for the LEDs at the end of the transfer */
    reset_delay_alarm_id = alarm_pool_add_alarm_in_us(alarm_pool_ws2812, RESET_TIME_US, reset_delay_complete, NULL, true);
  #else
...
void WS2812_PIO::dma_init(PIO _pio, unsigned int sm) {
  ...
  channel_config_set_dreq(&channel_config, pio_get_dreq(_pio, sm, true)); /* configure data request. true: sending data to the PIO state machine */
  ...
  dma_channel_configure(DMA_CHANNEL,
                        &channel_config,
                        &_pio->txf[sm], /* write address: write to PIO FIFO */
                        NULL, /* don't provide a read address yet */
                    #if NEOC_NOF_LANES>1
                        _led_count*2*NEOC_NOF_COLORS, /* number of transfers */
                    #else
                        _led_count, /* number of transfers */
                    #endif
...
bool WS2812_PIO::WS2812_DMATransferIsOngoing(void) {
#if NEOC_USE_DMA
  #if WS2812_USE_PICO_SEMAPHORE
...
void WS2812_PIO::WS2812_WaitForBufferReady(void) {
...
int WS2812_PIO::WS2812_Transfer(uint32_t address, size_t nofBytes) {
#if NEOC_NOF_LANES>1
  #if NEOC_USE_DMA
    #if WS2812_USE_PICO_SEMAPHORE
    ...
    for(int i=0; i<nofBytes/sizeof(uint32_t); i++) { /* without DMA: writing one after each other */
      pio_sm_put_blocking(pio0, _WS2812_sm, *p);
      p++;
    }
    ...
    dma_channel_set_read_addr(DMA_CHANNEL, (void*)address, true); /* trigger DMA transfer */
  #else
    for(int i=0; i<_led_count; i++) { /* without DMA: writing one after each other */
...
WS2812_PIO::WS2812_PIO(PinName pin, uint32_t led_count) {
  _pio = pio0; /* the PIO used */
  _WS2812_sm = 0; /* state machine used */
  _pin_num = static_cast<unsigned int>(pin);
  _led_count = led_count;
#if NEOC_NOF_LANES>1
  uint offset = pio_add_program(_pio, &ws2812_parallel_program); /* add program and get offset */
  ws2812_parallel_program_init(_pio, _WS2812_sm, offset, NEOC_PIN_START, NEOC_NOF_LANES, 800000); /* initialize it for 800 kHz */
#else /* multiple lanes */
  _offset = pio_add_program(_pio, &ws2812_program);
  ws2812_program_init(_pio, _WS2812_sm, _offset, _pin_num, 800000, NEOC_NOF_COLORS==4); /* initialize it for 800 kHz */
#endif
#if NEOC_USE_DMA
  #if WS2812_USE_PICO_SEMAPHORE
