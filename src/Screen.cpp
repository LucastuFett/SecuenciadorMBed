...
void Screen::left() {
    switch (mainState) {
        case MEMORY:
        case RENAME:
            if (_edit == 1) {  // Move left through letters
                _typePointer--;
                if (_typePointer < 0) _typePointer = 0;
            } else if (_edit == 2) {  // Change current letter/special
                if (_curPointer == 0) {
                    _letterPointer--;
...
void Screen::right() {
    switch (mainState) {
        case MEMORY:
        case RENAME:
            if (_edit == 1) {  // Move right through letters
                _typePointer++;
                if (_typePointer > 17) _typePointer = 17;
            } else if (_edit == 2) {  // Next letter/special
                if (_curPointer == 0) {
                    _letterPointer++;
...
void Screen::coverTitle() {
    ...
    if (_lastState != PROG && _lastState != NOTE && _lastState != CHANNEL && _lastState != TEMPO && _lastState != SCALE)
    ...
    switch (menu) {
        case 0:  // All Menus
            ...
            if (!tempo[0])
                ...
                puts(tempo[1] == 0 ? "Half" : tempo[1] == 1 ? "Normal"
                                          : tempo[1] == 2   ? "Double"
                                                            : "Err");
...
void Screen::paintScales() {
    ...
    if (pos != -1)
        fillrect(72, 201 - (_possible[0][pos] * 8) - 4 - offset, 78, 201 - (_possible[0][pos] * 8) - offset, Green);
    else
        fillrect(72, 201 - (note * 8) - 4 - offset, 78, 201 - (note * 8) - offset, Orange);
            ...
            for (uint8_t j = 0; j < 24; j++) {
                ...
                if ((beatsPerTone[bptIndex] & beatMask) != 0) {
                    if (count(_possible[0].begin(), _possible[0].end(), indNote)) {
                        ...
                        else if (indNote == _possible[0][0])
                            fillrect(xStart, yStart, xEnd, yEnd, Magenta);
                        else
                            fillrect(xStart, yStart, xEnd, yEnd, Blue);
                    } else {
                        if (note == indNote && octave == gridOctave)
                    ...
                    fillrect(xStart, yStart, xEnd, yEnd, Black);
            ...
            if ((i.first.channel == channel) && (i.first.midiNote >= (_curOctave * 12 + 24) && i.first.midiNote < (_curOctave + 2) * 12 + 24)) {
                ...
                for (gridBeat = firstBeat; gridBeat <= endBeat; gridBeat++) {
                    ...
                    if (count(_possible[0].begin(), _possible[0].end(), holdNote)) {
                        if (note == holdNote && octave == gridOctave)
                            fillrect(xStart, yStart, xEnd, yEnd, Green);
                        else if (holdNote == _possible[0][0])
                            fillrect(xStart, yStart, xEnd, yEnd, Magenta);
                        else
                            fillrect(xStart, yStart, xEnd, yEnd, Blue);
                    } else {
...
void Screen::updateMemoryText() {
    ...
    if (name != "" && _edit == 0) {
        ...
        if (_curPointer == 0) {
            if (_upper == 0)
                _typing[_typePointer] = letters[_letterPointer];
            else
                _typing[_typePointer] = shLetters[_letterPointer];
        } else
            _typing[_typePointer] = special[_specialPointer];
        ...
        if (!launchType) {  // Launchpad
            ...
            if (_lastLaunchOctave != launchOctave || _lastLaunchType != launchType) {
                for (uint8_t i = 0; i < 8; i++) {
                    ...
                    locate(8 + i * 39, 171);
                    if (i == 7)
                        puts((keyLabels[i + 8] + to_string(launchOctave + 1)).c_str());
                    else
                        puts((keyLabels[i + 8] + to_string(launchOctave)).c_str());
                    background(Black);
                }
                _lastLaunchType = launchType;
            }
