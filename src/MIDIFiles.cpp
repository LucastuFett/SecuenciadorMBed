...
// VID_0703&PID_0104
MIDIFiles::MIDIFiles() : BlockDeviceHolder(),
                         USBMSD(get_usb_phy(), _blockDevice, 0x0703, 0x0104, 1),
                         _fs("fs") {
    int err = _fs.mount(_blockDevice);
    if (err) {
        err = _fs.reformat(_blockDevice);
    }

    if (err) {
        while (1);  // Fatal error if even reformat fails
    }
...
void MIDIFiles::saveToFile() {
    ...
    if (mode < 2) {
        ...
        if (modeMIDI == 0)
            toneMIDI = majorConversion[tone];  // Major
        else
            toneMIDI = minorConversion[tone];  // Minor
    }
                ...
                if (buffer == 0xFF) {
                    ...
                    switch (buffer) {
                        ...
                        case 0x59: {
                            ...
                            mode = tempMode;
                            if (mode == 0)
                                tone = majorConversion[tempTone + 7];
                            else if (mode == 1)
                                tone = minorConversion[tempTone + 7];
                            chunklen -= 4;
                            break;
                        }
                    }
                    ...
                    if (msg[2] != 0) {
                        ...
                        if (type == 0)
                            ...
                            messageCount[curBeat]++;
                    } else {
                        // TODO: Only works with 32 beats
                        if (type == 0)
                            curOffMsg++;
                        else
                            offMessageCount[curBeat]++;
