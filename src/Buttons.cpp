...
void Buttons::press(uint8_t num) {
    switch (mainState) {
        ...
        case SCALE: {
            ...
            holdedMutex.lock();
            // If active
            if ((beatsPerTone[bptIndex] & beatMask) != 0 && hold == 0) {
                channels[channel]--;
                while (i < 10) {
                    ...
                    if (((midiMessages[index][0] & 0xF) == channel) && (midiMessages[index][1] == midiNote)) {
                        ...
                        if (beat >= origNum && beat <= num) timer.send(0x90 | channel, midiNote, 0);
            ...
            // If not Active
                ...
                if (!checkIfNotHolded) {
                    ...
                    for (i = 0; i < 10; i++) {
                        uint16_t index = (i * 32) + num;
                        if (hold != 2 && (beatsPerTone[bptIndex] & beatMask) == 0) {
                            if (midiMessages[index][0] == 0) {
                                ...
                                for (uint8_t j = 0; j < 10; j++) {
                                    if (offMessages[j * 32 + offNum][0] == 0) {
                                        offMessages[(j * 32) + offNum][0] = 0x90 | channel;
                                        offMessages[(j * 32) + offNum][1] = midiNote;
                                        offMessages[(j * 32) + offNum][2] = 0;
                                        break;
                                    }
                                }
                                beatMask = 0x80000000 >> offNum;
                                if ((control & beatMask) == 0) control |= beatMask;
                                break;
                            }
                        } else {
                            if (!checkIfNotAfterHold && num > _holdTemporary && (beatsPerTone[bptIndex] & beatMask) == 0) {
                                index = (i * 32) + offNum;
                                if (offMessages[index][0] == 0) {
                                    holdKey insertedKey = {_holdTemporary, uint8_t(channel), midiNote};

                                    offMessages[index][0] = 0x90 | channel;
                                    offMessages[index][1] = midiNote;
                                    offMessages[index][2] = 0;
                                    midiMessages[_holdIndex][0] = 0x90 | channel;
                                    midiMessages[_holdIndex][1] = midiNote;
                                    midiMessages[_holdIndex][2] = velocity;

                                    if (offNum > _holdTemporary) {
                                        holded.emplace(insertedKey, offNum - 1);
                                    } else if (offNum == 0) {
                                        if (mode32)
                                            holded.emplace(insertedKey, 31);
                                        else
                                            holded.emplace(insertedKey, 15);
                                    }
                                    hold = 0;
                                    beatMask = 0x80000000 >> _holdTemporary;
                                    if ((control & beatMask) == 0) control |= beatMask;
                                    beatMask = 0x80000000 >> offNum;
                                    if ((control & beatMask) == 0) control |= beatMask;
                                    break;
                                }
                            } else if (num == _holdTemporary) {
                                index = (i * 32) + offNum;
                                if (offMessages[index][0] == 0) {
                                    offMessages[index][0] = 0x90 | channel;
                                    offMessages[index][1] = midiNote;
        ...
        case DAW:
            if (!_DAWSent[num])
                timer.send(launchMessages[num][0], launchMessages[num][1], launchMessages[num][2]);
            else
                timer.send(launchMessages[num][0], launchMessages[num][1], 0);
...
void Buttons::updateColors() {
    ...
    switch (mainState) {
        ...
        case LAUNCH:
            for (uint8_t i = 0; i < 16; i++)
                if (keys[(i / 4) + 1][i % 4]) ledData[i] = launchColors[i];
            break;
        case DAW:
            for (uint8_t i = 0; i < 16; i++)
                if (_DAWSent[i]) ledData[i] = launchColors[i];
            break;
        default:
            break;
    }
